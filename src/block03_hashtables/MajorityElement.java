...
public class MajorityElement {
     ...
     * Первый элемент выдвигается кандидатом сразу, до цикла, поэтому цикл
     * начинается с индекса 1, а счётчик — с единицы: голос за первый элемент
     * уже подан. Инициализация тут не формальность ради компилятора,
     * а настоящий первый шаг алгоритма.
     * Пустой массив не обрабатывается намеренно: по ограничениям задачи длина
     * не меньше единицы. Вариант без этой гарантии — отдельный класс
     * {@link MajorityElementNotGuaranteed}.
     * Алгоритм и структура кода взяты из книги Макдауэлл (задача 17.10):
     * развёрнутый if/else — оттуда.
    ...
    public static int majorityElementByVoting(int[] nums) {
        ...
        for (int currentIndex = 1; currentIndex < nums.length; currentIndex++) {
            ...
            if (survivingVotes == 0) {
                /* Голоса за прежнего кандидата выбиты — выдвигаем нового. */
