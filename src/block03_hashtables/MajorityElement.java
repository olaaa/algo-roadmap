...
public class MajorityElement {
     ...
     * majority — значение, которое сейчас считается возможным доминирующим.
     * count — сколько его вхождений ещё не погашено чужими элементами.
     * Каждый элемент, не равный majority, взаимно выбывает с одним его
     * вхождением. Мажоритарный элемент занимает строго больше половины
     * массива, поэтому чужих на все его вхождения не хватает, и хотя бы одно
     * остаётся несокращённым.
     * ВАЖНО: по ходу цикла majority ответом быть не обязан. Гарантия
     * относится только к состоянию после последней итерации.
     * Код взят из книги Макдауэлл (задача 17.10) БЕЗ ИЗМЕНЕНИЙ: имена
     * переменных, развёрнутый if/else, инициализация нулём — всё как в тексте.
     * Однобуквенное n оставлено намеренно, вопреки скиллу java-meaningful-names:
     * при сверке с книгой код должен читаться один в один.
     */
    public static int majorityElementByVoting(int[] nums) {
        ...
        for (int n : nums) {
            if (count == 0) {
                /* Нет доминирующего элемента — начинаем заново. */
                majority = n;
            }
            if (n == majority) {
                count++;
    ...
    public static void main(String[] args) {
         ...
         *   5) голосование: count == 0, смена majority  [2,1,1]
         *   6) голосование: value == majority, count++  [1,1,1]
         *   7) голосование: value != majority, count--  [1,2,1]
        ...
        check(majorityElementByVoting(new int[]{5, 9, 9, 9, 9}) == 9,
              "голосование: промежуточное значение majority не ответ");

        /*
         * Пустой массив. По ограничениям задачи n >= 1, то есть такого входа
         * не бывает, но начальное значение majority определяет, упадёт метод
         * или нет. Инициализация nulём (как в книге) позволяет дойти
         * до конца цикла, который просто не начнётся.
         * Инициализация вида majority = nums[0] бросала бы здесь
        ...
        check(majorityElementByVoting(new int[]{}) == 0,
              "пустой массив: возвращается начальное значение majority");
