...
`PriorityQueue` — **неограниченная** очередь: предел числа элементов ей не задаётся, внутренний массив расширяется по мере надобности (как именно — в разделе «Что внутри в JDK 25»). Отсюда следствие для двух методов добавления: `offer` всегда возвращает `true`, а `add` никогда не бросает `IllegalStateException`, то есть по поведению они здесь неотличимы. Число в конструкторе `new PriorityQueue<>(2)` задаёт начальную ёмкость массива, а не предел. Деление очередей на ограниченные и неограниченные разобрано в [`Queue.md`](Queue.md).

Проверено запуском:

```
пять offer при начальной ёмкости 2 : true true true true true, размер 5
тысяча add подряд                  : исключения нет, размер 1005
```
...
1. дерево **законченное** (complete): все уровни заполнены целиком, кроме последнего, а последний заполняется слева направо, без пропусков внутри. Это одно из трёх похожих слов про форму дерева, и различаются они в [словарике](../glossary.md);
2. выполняется **свойство кучи** (heap property): значение каждого узла не больше значений его потомков.

Свойство кучи слабее, чем свойство дерева поиска: оно связывает узел только с его потомками, а между соседями слева и справа порядка нет. Поэтому куча не даёт отсортированный обход, зато поддерживается дешевле.

![Куча из шести элементов: дерево и тот же набор в массиве](img/pq-heap-and-array.svg)

*Рис. 1. Куча после `offer` в порядке 5, 3, 8, 1, 9, 2: сверху дерево, снизу тот же набор массивом. Серые числа рядом с узлами и под ячейками — индексы в массиве. Свойство кучи видно на каждом узле: родитель не больше своих потомков. Оранжевым отмечены узел `2` и его левый потомок `8` — узел стоит на индексе 2, потомок на индексе 2 · 2 + 1 = 5.*
...
деление целочисленное. Это в точности «полное представление» из [`TreeNotation.md`](../problems/block06_trees_graphs/TreeNotation.md), и здесь оно работает без оговорок — именно потому, что куча законченная. В записи из условия задачи применять эту арифметику нельзя: там у пустых узлов места под детей не зарезервированы, и после первого же внутреннего пропуска индексы перестают сходиться.

В исходнике `PriorityQueue` те же формулы записаны сдвигами: родитель — `(k - 1) >>> 1`, левый потомок — `(k << 1) + 1`, правый — `child + 1`.

Законченность даёт и высоту. Если `n` — число узлов, `h` — высота дерева в узлах (как принято в [`BinaryTree.md`](BinaryTree.md)), то `h = ⌊log₂ n⌋ + 1`. На картинке выше `n = 6`, и высота равна трём. Отсюда все `O(log n)`: и подъём, и спуск проходят один путь от корня к листу.
...
Новый элемент кладётся в конец массива — это единственное место, где законченность не нарушается, — и поднимается, пока он меньше родителя.
...
Меняться нужно именно с МЕНЬШИМ потомком: если поменяться с большим, тот окажется выше меньшего и свойство кучи сломается на следующем же уровне.
...
**Почему `offer` стоит `O(log n)`?** Элемент проходит путь от листа к корню, а высота законченного дерева из `n` узлов равна `⌊log₂ n⌋ + 1`.
...
`src/demo/PriorityQueueDemo.java` — все выводы из этого файла получены оттуда: порядок массива против порядка выдачи, компаратор, равные приоритеты, «k самых крупных», слияние выписок, неограниченность очереди, запрет `null`.
