...
Три формулы, которые пригодятся дальше. На уровне с номером `k` (корень — уровень 1) помещается до 2ᵏ⁻¹ узлов. Полное дерево из `h` уровней, то есть высоты `h`, содержит 2ʰ − 1 узлов. Дерево из `n` узлов имеет высоту от ⌈log₂ (n + 1)⌉ до `n`. Откуда они берутся — ниже, в разделе про глубину, высоту и уровень.
...
Глубина и высота считаются в противоположные стороны: глубина — от корня вниз к узлу, высота — от листа вверх к узлу. Так её и считает `maxDepth`: сначала ответ появляется у листьев, потом у их родителей, последним у корня.
...
высоте узла или о глубине дерева с корнем в этом узле — это одно и то же число.

Откуда формулы под рис. 1. У двоичного дерева каждый узел даёт не больше двух потомков, поэтому уровень не может быть
шире удвоенного предыдущего: на глубине `k` помещается до `2ᵏ⁻¹` узлов (1, 2, 4, 8, …). Полное дерево высоты `h`
содержит `2ʰ − 1` узлов: `1 + 2 + 4 + … + 2ʰ⁻¹`. Отсюда границы высоты для дерева из `n` узлов: самое плотное дерево
имеет высоту `⌈log₂(n + 1)⌉` — для `n = 7` это 3, для `n = 10⁴` всего 14, — а самое вытянутое, цепочка, высоту `n`.
Именно поэтому оценки памяти обходов ниже даны через высоту и ширину, а не через `n`: на одном и том же `n` они могут
отличаться в тысячи раз.
...
*Рис. 3. Симметричный порядок (in-order): узел посещается между левым и правым поддеревом. Порядок
`4, 2, 5, 1, 6, 3, 7`*

![Обратный порядок](img/dfs-postorder.svg)

*Рис. 4. Обратный порядок (post-order): узел посещается при возврате, после обоих поддеревьев. Порядок
`4, 5, 2, 6, 7, 3, 1`*
...
*Рис. 5. Слева дерево: у узла `4` один родитель, `preOrder` даёт `1, 2, 4, 3`. Справа то же с ребром `3 → 4`: у узла `4`
...
в ширину. Симметричный — только если дерево поиска.

Про память. Обход в глубину держит в стеке путь от корня до текущего узла и платит высотой. Обход в ширину держит в
очереди целый слой и платит шириной. На цепочке из n узлов высота n, а ширина 1 — в глубину дорого, в ширину дёшево. На
полном дереве наоборот: высота около `log₂ n`, а нижний слой — около `n / 2` узлов. Какой обход экономнее, зависит от
формы дерева, единого ответа нет.
...
Явный стек и стек вызовов стоят одинаково: и там и там хранится путь от корня до текущего узла. Разница только в том,
где лежит предел — у стека вызовов это размер стека потока (`-Xss`), у `ArrayDeque` — куча.
