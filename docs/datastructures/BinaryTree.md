...
`n = 2ʰ − 1` — для идеального дерева, где все уровни заполнены целиком.  
`⌈log₂(n + 1)⌉ ≤ h ≤ n` — законченное дерево слева, цепочка справа.
...
Ещё три слова, которые путают, — про ФОРМУ дерева: законченное, полное и идеальное. Они означают разное, и различаются
в [словарике](../glossary.md). Ниже встретится «идеальное дерево» — то, у которого все уровни заполнены целиком.
...
Вторая: `n = 2ʰ − 1` для идеального дерева. Это сумма всех уровней: `n = 1 + 2 + 4 + … + 2ʰ⁻¹ = 2ʰ − 1`. На рис. 1
`h = 3`, `n = 2³ − 1 = 7`.

Третья: `⌈log₂(n + 1)⌉ ≤ h ≤ n`. Это вторая формула, решённая относительно `h`. Из `n = 2ʰ − 1` следует `n + 1 = 2ʰ`,
откуда `h = log₂(n + 1)` — точное равенство для идеального дерева: `n = 7`, `h = log₂ 8 = 3`. Если последний уровень
заполнен не до конца, `n` лежит между двумя идеальными деревьями, `2ʰ⁻¹ − 1 < n ≤ 2ʰ − 1`, логарифм получается дробным,
а уровней — целое число, ближайшее сверху: `h = ⌈log₂(n + 1)⌉`. Для `n = 5` это `h = ⌈log₂ 6⌉ = ⌈2,58⌉ = 3` — пять
узлов в два уровня не уместить, там `n ≤ 3`. Для `n = 10⁴` выходит `h = 14`. Это левая граница, законченное дерево.
...
законченном дереве наоборот: `h ≈ log₂ n`, а нижний слой `w ≈ n / 2`. Какой обход экономнее, зависит от
...
| узлов в идеальном дереве высоты `h` | `n = 2ʰ − 1`                     | `n = 2ʰ⁺¹ − 1`      |
...
n, второе 1; у законченного дерева наоборот, log n против n / 2.
