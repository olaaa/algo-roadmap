...
Три формулы, которые пригодятся дальше. На уровне с номером `k` (корень — уровень 1) помещается до 2ᵏ⁻¹ узлов. Полное дерево из `h` уровней, то есть высоты `h`, содержит 2ʰ − 1 узлов. Дерево из `n` узлов имеет высоту от ⌈log₂ (n + 1)⌉ до `n`. Откуда они берутся — ниже, в разделе «Откуда берутся три формулы».
...
Речь о формулах под рис. 1. У двоичного дерева каждый узел даёт не больше двух потомков, поэтому уровень не может быть
