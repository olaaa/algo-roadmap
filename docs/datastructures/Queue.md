...
*Рис. 1. Очередь FIFO: вход в хвост, выход из головы*
...
Расходятся эти варианты только тогда, когда операцию выполнить нельзя: для снятия и просмотра такой случай — пустая очередь, для добавления — полная. Полной при этом может стать не всякая очередь, поэтому первую строку таблицы читают вместе с делением реализаций на ограниченные и неограниченные.

**Ограниченная очередь** — та, которой предел числа элементов задан числом в конструкторе и которая за этот предел не выходит: `ArrayBlockingQueue(2)`, `LinkedBlockingQueue(2)`, `LinkedBlockingDeque(2)`. Только у них добавление может не состояться, и только когда предел достигнут: тогда `add` бросает `IllegalStateException` (сообщение `Queue full`, а у деки `Deque full`), а `offer` возвращает `false`. Значение самого элемента тут ни при чём — исключение вызывает не то, что кладут, а то, что класть некуда.

**Неограниченная очередь** растёт, пока хватает памяти: `ArrayDeque`, `LinkedList`, `PriorityQueue`, `ConcurrentLinkedQueue`, а также `LinkedBlockingQueue` и `LinkedBlockingDeque`, созданные без числа в конструкторе — у них предел равен `Integer.MAX_VALUE`, то есть 2 147 483 647 элементов. У таких очередей переполнение недостижимо: `offer` всегда возвращает `true`, `add` не бросает никогда, и правый столбец таблицы от левого не отличается.

Число в `new ArrayDeque<>(2)` или `new PriorityQueue<>(2)` предела не задаёт: это начальная ёмкость внутреннего массива, который потом расширяется. После пяти вставок в такую очередь в ней лежат все пять элементов.
...
Формулировка для собеседования про добавление: «`offer` придуман для ограниченных очередей, у неограниченных он неотличим от `add`».

## Реализации

### ArrayDeque — выбор по умолчанию

Кольцевой буфер. Быстрее всех для роли простой очереди. Реализует `Deque`, а `Deque` наследует `Queue`, поэтому годится и туда, и туда:
...
Устройство кучи, разбор подъёма и спуска по шагам и применение — от планировщиков в самой Java до очереди платежей — в отдельном справочнике [`PriorityQueue.md`](PriorityQueue.md).
