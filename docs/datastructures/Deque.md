...
кольцевой буфер,     двусвязный список,
...
Это второй по частоте вопрос после «чем отличается от Queue». Реализация — **кольцевой буфер** (в оригинале circular array).
...
Кольцевой буфер решает это так: **массив не трогаем вообще, вместо элементов двигаются границы**. Заводим два индекса — `head` (где лежит первый элемент) и `tail` (куда положить следующий с конца). Сняли с головы — просто увеличили `head`. Освободившаяся ячейка не заполняется дырой в структуре, она становится «зарезервированной под будущее».
...
![Кольцевой буфер](img/arraydeque-ring.svg)

*Рис. 4. Кольцевой буфер внутри ArrayDeque: head, tail и заворот через край*
...
В исходниках JDK и в англоязычной литературе термин один — **circular array**. По-русски единого перевода нет, но у самой структуры название есть: **кольцевой буфер** (он же циклический буфер) — так её называют русская Википедия и справочная литература, и здесь используется оно. В русском издании Макдауэлл (лежит в `rag/`) — **«циклический массив»**; встречается и калька «кольцевой массив». Все варианты означают одно и то же, на собеседовании поймут любой.
...
|-------|--------|-------------|
| `ArrayDeque` | кольцевой буфер | быстрый, компактный, **null запрещён**, выбор по умолчанию |
...
Кольцевой буфер с индексами `head` и `tail`, заворачивающимися через границу. Никаких сдвигов при работе с концами, все операции `O(1)` амортизированно.
